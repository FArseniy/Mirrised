...
> Приватная трансляция экрана из браузера: один зритель или P2P-группа до шести человек.
...
- три режима комнаты: P2P напрямую для одного зрителя, P2P с TURN и группа до шести зрителей;
...
- профили захвата от 720p/15 fps до 2K/60 fps и режим без ограничений браузера;
...
SDP и ICE-кандидаты сервер не разбирает. Он сверяет роль отправителя, комнату и
адресата: в группе ведущий может переслать сигнал только одному из зрителей этой
же комнаты.

## Режимы комнат

| Режим | Зрители | Маршрут |
| --- | ---: | --- |
| P2P напрямую | 1 | только STUN; при строгом NAT соединение может не установиться |
| P2P с TURN | 1 | прямой P2P с резервным TURN-маршрутом |
| Группа | до 6 | отдельное P2P-соединение с каждым зрителем; TURN — только при необходимости |

В группе исходящий трафик ведущего растёт пропорционально числу зрителей.
...
| Лимит участников по режиму комнаты | лишние зрители и неопределённые роли |
